...
# 120 not v midi
def mappingOld(value, midi):
    if value >= 120:
        return mappingOld(value % 10, midi)
    if value < 0:
        return mappingOld(int(value * 10), midi)
    for item in midi:
        if item == value: # vrne ce je enaka
            return value
        if item > value: # vrne prvo ki je vecja
            return item


def mapping(value, min, max):
    OldRange = max - min
    NewRange = 120 - 0
